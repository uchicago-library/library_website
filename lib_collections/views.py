...
def collections(request):
    ...
    if view == 'collections':
        ...
        # subject
        if subject:
            filter_arguments['collection_subject_placements__subject__in'] = Subject.objects.get(
                name=subject).get_descendants()

        # search
        if search:
            filter_arguments['id__in'] = list(
                map(lambda s: s.id, CollectionPage.objects.live().search(search)))
        ...
        ).select_related('thumbnail').prefetch_related('collection_subject_placements')

        # sort browses by title, omitting leading articles.
        if not search:
            collections = sorted(collections, key=lambda c: re.sub(
                r'^(A|An|The) ', '', c.title))
    ...
    if view == 'exhibits':
        ...
        if subject:
            filter_arguments['exhibit_subject_placements__subject__in'] = Subject.objects.get(
                name=subject).get_descendants()
        ...
        exhibits = ExhibitPage.objects.live().filter(**filter_arguments).distinct().select_related('thumbnail',
                                                                                                   'exhibit_location').prefetch_related('exhibit_subject_placements')
        exhibits_current = exhibits.filter(exhibit_open_date__lt=datetime.datetime.now(
        ).date(), exhibit_close_date__gt=datetime.datetime.now().date()).distinct()
        ...
        if not search:
            exhibits = sorted(exhibits, key=lambda e: re.sub(
                r'^(A|An|The) ', '', e.title))
            exhibits_current = sorted(
                exhibits_current, key=lambda e: re.sub('r^(A|An|The) ', '', e.title))

    # locations
    locations = sorted(list(set(ExhibitPage.objects.exclude(
        exhibit_location=None).values_list('exhibit_location__title', flat=True))))
    ...
    if view == 'subjects':
        ...
        subjects_with_collections = set(
            CollectionPageSubjectPlacement.objects.values_list('subject', flat=True))
        subjects_with_exhibits = set(
            ExhibitPageSubjectPlacement.objects.all().values_list('subject', flat=True))
        ...
        for s in subjects_queryset:
            if default_cache.has_key('subject_id_%s' % s.id):
                ...
                if not subject_descendants:
                    ...
                    default_cache.set('descendants_%s' %
                                      s.id, subject_descendants, 60*5)

                parents = qs.filter(child=s).order_by('parent__name').values_list(
