...
from lib_collections.models import (
    CollectionPage,
    CollectionPageSubjectPlacement,
    ExhibitPage,
    ExhibitPageSubjectPlacement,
    Format,
)
...
def collections(request):
    ...
    if view == "collections":
        ...
        if subject:
            filter_arguments["collection_subject_placements__subject__in"] = (
                Subject.objects.get(name=subject).get_descendants()
            )
    ...
    if view == "exhibits":
        ...
        exhibits = (
            ...
            .filter(**filter_arguments)
            .distinct()
            .select_related("thumbnail", "exhibit_location")
        ...
        for s in subjects_queryset:
            if ("subject_id_%s" % s.id) in default_cache:
                ...
                parents = (
                    ...
                    .values_list("parent__name", flat=True)
    ...
    subjects_pulldown = (
        Subject.objects.filter(display_in_dropdown=True)
        .order_by("name")
    ...
    return render(
        request,
        "lib_collections/collections_index_page.html",
        {
            ...
            "locations": locations,
            "search": search,
            "self": {"title": "Collections & Exhibits"},
            "sort": sort,
            "subject": subject,
            "subjects": subjects,
