...
class Command (BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        initial_objects = [
            self.get_object_from_tuple(t) for t in initial_tuples
        ]

        include_subclasses = []
        for m in self.include_fields.keys():
            for c in self.include_fields[m].keys():
                include_subclasses.append(
        ...
        for obj in initial_objects:
            ...
            if related:
                for field in obj._meta.fields:
                    ...
                    if not type(related_object).__module__ in self.include_fields.keys():
                        continue
                    if not type(related_object).__name__ in self.include_fields[type(related_object).__module__]:
                        continue
