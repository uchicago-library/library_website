...
(function () {
  ...
  function applyHtmlProperties(rules) {
    ...
    function getFirstHeadingTextWithin($elem, levels) {
    ...
    rules.forEach(function (rule) {
      ...
      if (rule.childSelector) {
        const $children = $items.find(rule.childSelector);
        if ($children.length) {
          $items = $children;
        } else { return; }
      }
      ...
      $items.each(function () {
        ...
        const $target = $(this); // the element matched by selector (the "main" element)
      ...
      checkbox(target, searchWidget) {
        ...
        if (searchButtons.length) {
          searchButtons.forEach((searchButton) => {
            const optionText = target.getAttribute('data-ga-label');
    ...
    getPositionInList(link, params) {
      if (!link.closest(SELECTORS.FOOTER)) {
        ...
        // Catalog VuFind Search results
          ...
          const vufindAncestor = link.closest('[data-record-number]');
          if (vufindAncestor) {
            params.click_position = vufindAncestor.getAttribute('data-record-number');
          }
        }
        // Guides Search results
          ...
          const guidesAncestor = link.closest('.s-srch-results');
          const item = link.closest('.s-srch-result');
          if (guidesAncestor && item && !link.closest('.pagination')) {
            params.click_position = Array.from(guidesAncestor.children).indexOf(item) + 1;
          }
        }
        // for the news list on the home page and on the news page
        else if ((ancestor = link.closest('.news-wrap, .news-stories'))) { // eslint-disable-line no-cond-assign
        ...
        else if ((ancestor = link.closest('tbody'))) { // eslint-disable-line no-cond-assign
