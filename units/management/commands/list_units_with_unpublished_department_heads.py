...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        units_with_unpublished_department_heads = UnitPage.objects.exclude(
            department_head=None
        ).filter(department_head__live=False)
        if units_with_unpublished_department_heads:
            print(
                "The following UnitPages have a department_head with an unpublished StaffPage."
            )
            print("")
