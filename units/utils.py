...
def get_quick_num_or_link(dic):
    ...
    assert all(
        k in dic for k in ["label", "number", "link"]
    ...
    if dic["link"]:
        ...
        retval = (
            1,
            dic["label"],
            Page.objects.all().get(id=dic["link"]).get_url_parts()[-1],
        )
    elif dic["number"]:
    ...
    if library:
        ...
        if dept_slug in quick_nums:
            ...
            try:
                ...
                url_path = (
                    UnitIndexPage.objects.first().url_path
                    + "/".join(map(slugify, department.split(" - ")))
                    + "/"
                )
...
class WagtailUnitsReport:
    ...
    def _units_out_of_sync(self):
        ...
        for u in UnitPage.objects.filter(display_in_campus_directory=True, live=True):
        ...
        missing_in_wagtail = sorted(list(wag_unit_names.difference(api_unit_names)))
    ...
    def _get_campus_directory_unit_names(self):
        ...
        for d in x.findall(".//departments/department"):
            department_name = re.sub(r"\s+", " ", d.find("name").text).strip()
            ...
            for d2 in x2.findall(".//subDepartments/subDepartment"):
                subdepartment_name = re.sub(r"\s+", " ", d2.find("name").text).strip()
    ...
    def _get_units_out_of_sync_data(self):
        ...
        if wagtail_units:
            output.append(
                ["THE FOLLOWING UNITS APPEAR IN WAGTAIL, BUT NOT THE UNIVERSITY'S API:"]
            )
        ...
        if campus_units:
            output.append(
                ["THE FOLLOWING UNITS APPEAR IN THE UNIVERSITY'S API, BUT NOT WAGTAIL:"]
            )
    ...
    def _get_units_wagtail(self):
        ...
        try:
            if self.options["latest_revision_created_at"]:
                ...
                new_unitpages = set(
                    UnitPage.objects.filter(
                        latest_revision_created_at__gte=latest_revision_created_at_string
                    )
                )
                unitpages = (
                    unitpages.intersection(new_unitpages)
                    if unitpages
                    else new_unitpages
                )
        except KeyError:
        ...
        try:
            if self.options["display_in_campus_directory"]:
                new_unitpages = set(
                    UnitPage.objects.filter(display_in_campus_directory=True)
                )
                unitpages = (
                    unitpages.intersection(new_unitpages)
                    if unitpages
    ...
    def _get_units_report_data(self):
        ...
        for u in unitpages:
            try:
                latest_revision_created_at = u.latest_revision_created_at.strftime(
                    "%m/%d/%Y %-I:%M:%S %p"
                )
