...
function _isEmpty(e){ // eslint-disable-line no-underscore-dangle
...
function ebscoHostSearchGo(form) { // eslint-disable-line no-unused-vars
  // Declare variables at function scope to avoid hoisting issues
  var ebscohosturl;
  var index;
  ...
  function ebscoAddDisciplines() {
    function addScrollBoxLimiters(disciplinesDiv) {
      ...
      var options = visibleDisciplines ? visibleDisciplines.getElementsByTagName('option') : [];
      var selectedOptionValues = [];
			
      if (options.length) {
        for (var i = 0; i < options.length; i++) {
          var theOption = options[i];
          var code = theOption.value;
        ...
        if (selectedOptionValues.length) {
          ...
          for (var j = 0; j< selectedOptionValues.length; j++) {
            ebscohosturl += 'LO+system.dis-' + selectedOptionValues[j].toLowerCase() + '%7e';
          }
        }
      ...
      function addDisciplinesToUrl () { // eslint-disable-line no-unused-vars
        var k;
        for (k = 0; k < selectedOptionValues.length; k++) {
          addToUrl(selectedOptionValues[k]);
        }
        ...
        for (var m = 0; m < hiddenDisciplines.length; m++ ) {
          addToUrl(hiddenDisciplines[m].value);
        }
    ...
    ebscohosturl = ebscohosturl.replace(/%7e$/, '');
	 ...
	 var ebscohostwindow = parseInt(chkObject(form.ebscohostwindow,0), 10);
	 ebscohosturl = chkObject(form.ebscohosturl,'http://search.ebscohost.com/login.aspx?');
    ...
    return false;
	 ...
	 index = (matches===null)?0:matches.length; 
  ...
  try {
    ...
    // Error in ebscoAddDisciplines - silently ignore
  }
...
function highlight(id, currentClass, checkboxId) {
  var checkboxElement = findObj(checkboxId);
  var rowElement = findObj(id);

  rowElement.className = checkboxElement.checked ? 'selected' : currentClass;
  if(checkboxElement.checked==false)
 		findObj('cball').checked=false;
}

function SelectAllCheckBoxes(e) { // eslint-disable-line no-unused-vars
  var cbs = document.getElementsByName("cbs");
  var rows = document.getElementsByClassName('choose-db-list')[0].children;
...
function findObj(n, d) {
  ...
  if(!d)
 		d=document;
  if((p=n.indexOf("?"))>0&&parent.frames.length) { // eslint-disable-line no-cond-assign
 		d=parent.frames[n.substring(p+1)].document;
 		n=n.substring(0,p);
  }

  if(!(x=d[n])&&d.all) // eslint-disable-line no-cond-assign
 		x=d.all[n]; 
