...
def spaces(request):
    ...
    spaces = (
        ...
        .order_by("title")
        .select_related("parent_building", "location_photo")
    )
    if building:
    ...
    if building:
        ...
        id_list = all_spaces.filter(parent_building__title=building).values_list(
            "pk", flat=True
        )
        ...
        floors = sorted(
            list(
                set(
                    LocationPageFloorPlacement.objects.filter(parent__in=id_list)
                    .exclude(floor=None)
    ...
    return render(
        ...
        "public/spaces_index_page.html",
        {
            ...
            "floor": floor,
            "floors": floors,
            "self": {"title": "Library Spaces", "friendly_name": friendly_name},
            "spaces": spaces,
