...
class StaffPageEmailAddresses(Orderable, models.Model):
    page = ParentalKey(
        "staff.StaffPage", on_delete=models.CASCADE, related_name="staff_page_email"
    )
...
class StaffPageLibraryUnits(Orderable, models.Model):
    page = ParentalKey(
        "staff.StaffPage", on_delete=models.CASCADE, related_name="staff_page_units"
    )
...
class StaffPage(BasePageWithoutStaffPageForeignKeys):
    ...
    first_name = CharField(
        blank=True, help_text="First name, for sorting.", max_length=255, null=True
    )
    middle_name = CharField(
        blank=True, help_text="Middle name, for sorting.", max_length=255, null=True
    )
    last_name = CharField(
        blank=True, help_text="Last name, for sorting.", max_length=255, null=True
    )
    ...
    supervises_students = BooleanField(default=False, help_text="For HR reporting.")
    ...
    def get_staff_by_building(building_str):
        ...
        if building > 0:
            return (
                ...
                .filter(staff_page_units__library_unit__building=building)
                .distinct()
            )
        else:
...
class StaffPageExpertisePlacement(Orderable, models.Model):
    page = ParentalKey(
        "staff.StaffPage", on_delete=models.CASCADE, related_name="expertise_placements"
    )
