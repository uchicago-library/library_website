...
class NewsPageTestCase(TestCase):
	...
	# in sticky stories and not in news stories.
        ...
        sticky_story_pks = NewsPage.get_stories(sticky=True, now=now).object_list.values_list('pk', flat=True)
        news_story_pks = NewsPage.get_stories(sticky=False, now=now).object_list.values_list('pk', flat=True)
